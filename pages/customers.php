    ...
    <div class="col-12">
        <div class="card">
            <div class="card-header d-flex justify-content-between align-items-center">
                <h3 class="card-title"><i class="fas fa-users me-2"></i>Müşteri Listesi</h3>
                <button class="btn btn-success btn-sm" onclick="openModal()"><i class="fas fa-plus me-1"></i>Müşteri
                    Ekle</button>
            </div>
            <div class="card-body">
...
<script>
    ...
    function openModal() { $('#formAction').val('add'); $('#formId').val(''); $('#crudForm')[0].reset(); $('#modalTitle').text('Müşteri Ekle'); $('#crudModal').modal('show'); }
    function editRow(id) { $.get(apiUrl, { action: 'get', id: id }, function (r) { if (!r.success) return showError(r.message); var u = r.data; $('#formAction').val('edit'); $('#formId').val(u.id); $('[name="name"]').val(u.name); $('[name="contact"]').val(u.contact); $('[name="email"]').val(u.email); $('[name="phone"]').val(u.phone); $('[name="address"]').val(u.address); $('[name="is_active"]').val(u.is_active); $('#modalTitle').text('Müşteri Düzenle'); $('#crudModal').modal('show'); }, 'json'); }
