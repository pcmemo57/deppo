    ...
    <div class="col-12">
        <div class="card card-warning card-outline">
            <div class="card-header">
                <h3 class="card-title text-bold"><i class="fas fa-users me-2"></i>Müşteri Listesi</h3>
                <div class="card-tools d-flex gap-2">
                    ...
                    <button class="btn btn-primary btn-sm px-3 shadow-sm" onclick="openModal()"
                        ...
                        <i class="fas fa-plus me-1"></i> Yeni Müşteri Ekle
                    </button>
                </div>
            </div>
    ...
    <div class="modal-dialog modal-lg">
        <div class="modal-content">
            ...
            <div class="modal-footer">
                ...
                <button type="button" class="btn btn-warning" id="btnSave"><i
...
<script>
    ...
    var apiUrl = '<?= BASE_URL ?>/api/customers.php';
    function esc(v) { return $('<span>').text(v || '').html(); }
    function load() { $.get(apiUrl, { action: 'list', page: curPage, per_page: curPerPage, search: curSearch }, function (r) { if (!r.success || !r.data.data) return; var html = ''; $.each(r.data.data, function (i, u) { html += '<tr><td><b>' + esc(u.name) + '</b></td><td>' + esc(u.contact || '—') + '</td><td>' + esc(u.email || '—') + '</td><td>' + esc(u.phone || '—') + '</td><td>' + (u.is_active == 1 ? '<span class="status-badge active" onclick="toggleRow(' + u.id + ',1)"><i class="fas fa-check"></i> AKTİF</span>' : '<span class="status-badge inactive" onclick="toggleRow(' + u.id + ',0)"><i class="fas fa-times"></i> PASİF</span>') + '</td><td><button class="btn btn-xs btn-info me-1" onclick="editRow(' + u.id + ')"><i class="fas fa-edit"></i></button><button class="btn btn-xs btn-danger" onclick="deleteRow(' + u.id + ')"><i class="fas fa-trash"></i></button></td></tr>'; }); $('#tableBody').html(html || '<tr><td colspan="6" class="text-center text-muted p-3">Kayıt bulunamadı</td></tr>'); $('#totalCount').text('Toplam: ' + formatQty(r.data.total) + ' kayıt'); renderPag(r.data.total); }, 'json'); }
    ...
    function openModal() { $('#formAction').val('add'); $('#formId').val(''); $('#crudForm')[0].reset(); setStatus(1); $('#modalTitle').text('Müşteri Ekle'); $('#crudModal').modal('show'); }
    function editRow(id) {
        $.get(apiUrl, { action: 'get', id: id }, function (r) {
            if (!r.success) return showError(r.message);
            var u = r.data;
            $('#formAction').val('edit'); $('#formId').val(u.id);
            $('[name="name"]').val(u.name); $('[name="contact"]').val(u.contact);
            $('[name="email"]').val(u.email); $('[name="phone"]').val(u.phone);
            $('[name="address"]').val(u.address);
            setStatus(u.is_active);
            $('#modalTitle').text('Müşteri Düzenle'); $('#crudModal').modal('show');
        }, 'json');
    }
    ...
    function deleteRow(id) { confirmAction('Bu müşteriyi silmek istediğinize emin misiniz?', function () { $.post(apiUrl, { action: 'delete', id: id }, function (r) { r.success ? showSuccess(r.message) : showError(r.message); load(); }, 'json'); }); }
    ...
    $('[name="email"]').on('blur', function () {
        ...
        $.get(apiUrl, { action: 'check_email', email: email, id: id }, function (r) {
            if (r.success && r.data.exists) {
                showError('Bu e-posta adresi zaten bir müşteri tarafından kullanılıyor.');
            }
        }, 'json');
    });
