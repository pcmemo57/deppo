        ...
        <div class="card card-primary card-outline">
            <div class="card-header">
                ...
                <div class="card-tools d-flex gap-1 align-items-center">
                    ...
                    <div class="ms-1 d-none d-md-block" id="grandTotalContainer">
                        <span class="badge bg-light text-primary border px-3 py-2 shadow-sm"
                            style="font-size: 14px; font-weight: 400;">
                            Genel Toplam: <span id="grandTotalValue" class="fw-bold">0,00</span>
                            <?= getCurrencySymbol() ?>
                        </span>
                    </div>
                </div>
                        ...
                        <li class="list-group-item d-flex justify-content-between align-items-center py-2 px-4 warehouse-item"
                            ...
                            <div class="premium-switch">
                                <input class="wh-switch d-none" type="checkbox" value="<?= e($w['id']) ?>"
                                    data-name="<?= e($w['name']) ?>" id="wh_<?= e($w['id']) ?>" checked>
...
<style>
    ...
    .card-footer .float-end {
        ...
        .wh-badge-0 { background-color: #e3f2fd !important; color: #0d47a1 !important; border: 1px solid #bbdefb
        !important; } /* Blue */
        .wh-badge-1 { background-color: #f1f8e9 !important; color: #33691e !important; border: 1px solid #dcedc8
        !important; } /* Green */
        .wh-badge-2 { background-color: #fff3e0 !important; color: #e65100 !important; border: 1px solid #ffe0b2
        !important; } /* Orange */
        .wh-badge-3 { background-color: #f3e5f5 !important; color: #4a148c !important; border: 1px solid #e1bee7
        !important; } /* Purple */
        .wh-badge-4 { background-color: #e0f2f1 !important; color: #004d40 !important; border: 1px solid #b2dfdb
        !important; } /* Teal */
        .wh-badge-5 { background-color: #fffde7 !important; color: #f57f17 !important; border: 1px solid #fff9c4
        !important; } /* Yellow */
        .wh-badge-6 { background-color: #fbe9e7 !important; color: #bf360c !important; border: 1px solid #ffccbc
        !important; } /* Deep Orange */
        .wh-badge-7 { background-color: #e1f5fe !important; color: #01579b !important; border: 1px solid #b3e5fc
        ...
        <script>
            ...
            function load() {
                ...
                $.get(apiUrl, { action: 'list', page: curPage, per_page: curPerPage, search: curSearch, warehouses: whs.join(',') }, function (r) {
                    ...
                    // Thead
                    var headHtml = '<tr><th>Ürün</th><th>Depo</th><th class="num-align" style="width:160px">Stoktaki Miktar</th><th class="num-align" style="width:140px">Toplam Eder</th><th style="width:80px" class="text-center">İşlemler</th></tr>';
                    ...
                    $.each(r.data.data, function (i, row) {
                        ...
                        $.each(r.data.columns, function (j, col) {
                            ...
                            var rowTotalValue = qty * (row.last_price_base || 0);
                            grandTotal += rowTotalValue;

                            html += '<tr class="' + (isLowStock ? 'table-warning' : '') + '">';
                            html += '<td>';
                            if (isLowStock) {
                                html += '<span class="badge bg-danger" style="margin-right: 10px;" title="Global Stok Alarm Seviyesi Altında!"><i class="fas fa-exclamation-triangle"></i></span>';
                            }
                            if (row.image) {
                            ...
                            html += '<td class="num-align fw-bold text-primary">' + formatPrice(rowTotalValue) + ' <small class="text-muted fw-normal">' + currencySymbol + '</small></td>';
                    ...
                    $('#grandTotalValue').text(formatPrice(grandTotal));
            ...
            $('#btnExcel').on('click', function () {
                ...
                // 3. Table Header
                ws_data.push(["Ürün Adı", "Stoktaki Miktar", "Depo", "Birim Fiyat", "Toplam Eder"]);

                // 4. Data Rows
                var excelGrandTotal = 0;
                $.each(currentData, function (i, row) {
                    $.each(currentCols, function (j, col) {
                        var qty = row.stocks[col] || 0;
                        if (qty == 0) return;
                        var price = row.last_price_base || 0;
                        var rowTotal = qty * price;
                        excelGrandTotal += rowTotal;
                        ws_data.push([row.product, qty, col, price, rowTotal]);
                    });
                });

                // 5. Total Row
                ws_data.push([""]);
                ws_data.push(["GENEL TOPLAM", "", "", "", excelGrandTotal]);
                ...
                for (var R = range.s.r; R <= range.e.r; ++R) {
                    for (var C = range.s.c; C <= range.e.c; ++C) {
                        ...
                        if (R === 0) {
                            ...
                            if (C === 1 || C === 3 || C === 4) ws[cell_ref].s = styleDataNum;
