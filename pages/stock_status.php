        ...
        $.get(apiUrl, { action: 'list', page: curPage, per_page: curPerPage, search: curSearch, warehouses: whs.join(',') }, function (r) {
            ...
            $.each(r.data.data, function (i, row) {
                ...
                $.each(r.data.columns, function (j, col) {
                    ...
                    var whAlarm = row.warehouse_alarms ? row.warehouse_alarms[col] : 0;
                    var isWhLow = (whAlarm > 0 && qty < whAlarm);

                    var rowIndexText = rowIndex++;
                    var imgSrc = row.image ? '<?= BASE_URL ?>/images/UrunResim/' + encodeURIComponent(row.image) : '';
                    var qtyClass = qty < 0 ? 'text-danger fw-bold' : (isWhLow ? 'text-danger fw-bold' : 'text-success text-bold');

                    var rowTotalValue = qty * (row.last_price_base || 0);
                    grandTotal += rowTotalValue;

                    html += '<tr class="' + (isLowStock ? 'table-warning' : (isWhLow ? 'table-danger' : '')) + '">';
                    html += '<td>';
                    if (isLowStock) {
                        html += '<span class="badge bg-warning text-dark" style="margin-right: 5px;" title="Global Stok Alarm Seviyesi Altında!"><i class="fas fa-bell"></i></span>';
                    }
                    if (isWhLow) {
                        html += '<span class="badge bg-danger" style="margin-right: 5px;" title="Depo Bazlı Alarm Seviyesi Altında! (' + whAlarm + ')"><i class="fas fa-exclamation-circle"></i></span>';
                    }
                    if (row.image) {
