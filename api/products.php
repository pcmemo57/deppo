...
switch ($action) {
    ...
    case 'add':
        ...
        Database::insert("INSERT INTO `$table` (name,code,unit,description,image,is_active) VALUES (?,?,?,?,?,?)",
        [$name, sanitize($_POST['code'] ?? ''), sanitize($_POST['unit'] ?? 'Adet'),
            sanitize($_POST['description'] ?? ''), $imageFile, (int)($_POST['is_active'] ?? 1)]);
    ...
    case 'edit':
        ...
        Database::execute("UPDATE `$table` SET name=?,code=?,unit=?,description=?,image=?,is_active=? WHERE id=?",
        [$name, sanitize($_POST['code'] ?? ''), sanitize($_POST['unit'] ?? 'Adet'),
            sanitize($_POST['description'] ?? ''), $imageFile, (int)($_POST['is_active'] ?? 1), $id]);
    ...
    case 'search_select2':
        ...
        $results = array_map(function ($r) {
            return ['id' => $r['id'], 'text' => $r['name'] . ($r['code'] ? ' [' . $r['code'] . ']' : ''),
            'image' => $r['image'], 'unit' => $r['unit']];
        }, $rows);
