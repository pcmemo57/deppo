...
switch ($action) {
    ...
    case 'edit':
        ...
        if (!$id || !$name)
            jsonResponse(false, 'Tedarikçi adı zorunludur.');
        Database::execute("UPDATE `$table` SET name=?,contact=?,email=?,phone=?,address=?,is_active=? WHERE id=?", [$name, sanitize($_POST['contact'] ?? ''), sanitize($_POST['email'] ?? ''), sanitize($_POST['phone'] ?? ''), sanitize($_POST['address'] ?? ''), (int) ($_POST['is_active'] ?? 1), $id]);
        jsonResponse(true, 'Tedarikçi güncellendi.');
    case 'toggle':
        $id = (int) ($_POST['id'] ?? 0);
    ...
    case 'delete':
        ...
        if (hasMovement($table, 'id', $id)) {
            ...
            jsonResponse(true, 'Tedarikçi gizlendi (hareketi olduğu için silinemez).');
        }
